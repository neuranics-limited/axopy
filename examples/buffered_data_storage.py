...
class MyTask(Task):
    ...
    def prepare_daq(self, daqstream):
        ...
        self.timer = Counter(50000)  # number of daq read cycles
    ...
    def run_trial(self, trial):
        ...
        """
        Replaces:
        trial.add_array('name', stack_axis=1)
        """

        # 8 channels, 50000 daq reads * 20 read_size
        bdims = (8, 50000 * 20)

        trial.add_bufferedarray('data_raw',  insert_axis=1, buffer_dims=bdims)
        trial.add_bufferedarray('data_proc',  insert_axis=1, buffer_dims=bdims)
    ...
    def update(self, data):
        ...
        """
        Replaces:
        trial.arrays['name'].stack(data)
        """

        # Update Arrays
        self.trial.arrays['data_raw'].insert(data)
        self.trial.arrays['data_proc'].insert(data_proc)
    ...
    def finish_trial(self):
        ...
        New:
        """
        self.trial.arrays['data_raw'].set_data()
        self.trial.arrays['data_proc'].set_data()
...
if __name__ == '__main__':
    ...
    dev = NoiseGenerator(rate=2000, num_channels=8, amplitude=1.0,
                         read_size=20)
